...
- **Handling external service errors (9)**.  
The service implements error handling methods and postponing requests in cases where the remote server is unavailable/takes a long time to respond to a request/returns incorrect responses. These methods do not allow you to influence the performance of the server in cases of delays in external services caused by the above failures.  
This behavior is achieved using:
    - `smsending.service.validation_response`
    - loop in method: `smsending.service.serv_send`
